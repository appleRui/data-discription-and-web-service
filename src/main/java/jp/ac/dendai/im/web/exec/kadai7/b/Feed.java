...
public class Feed {
	...
	/**
	 * keywordに一致したタイトルを持っている配列
	 * @param keyword string
	 * @return List<Item>
	 */
	public List<Item> getTargetKeywordItemList(String keyword){
		ArrayList<Item> itemList = this.getItemList();
		return itemList.stream().filter(item -> item.getTitle().contains(keyword)).collect(Collectors.toList());
	}
	...
	public ArrayList<Item> getItemList() {
		...
		try {
			...
			for(int i = 0; i < itemNodeList.getLength(); i++) {	// 各item要素について
				...
				itemList.add(new Item(id, title, link, description, creator));
			}
		}
		catch (Exception e) {
			e.printStackTrace();
